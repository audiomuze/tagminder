...
def setup_logging(verbose=False, execute=False):
    ...
    if execute:
        log_filename = f"{get_script_name(with_extension=False)}.log"
...
def update_database_and_changelog(
    ...
    script_name = get_script_name(with_extension=True)
    ...
    for old_path, new_path, rowid in updates:
        try:
            ...
            if old_path != new_path:
                ...
                actual_updates += 1
                logging.info(f"Updated database for rowid {rowid}: {old_path} -> {new_path}")
            else:
                logging.debug(f"No change needed for rowid {rowid}: {old_path}")
    ...
    conn.commit()
    logging.info(f"Database update completed: {actual_updates} records updated out of {len(updates)} processed")
...
def main():
    ...
    script_name = get_script_name(with_extension=False)
    actual_script_name = get_script_name(with_extension=True)
