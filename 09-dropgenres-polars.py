...
Purpose:
    ...
    removing any entries that don't appear in the table _ref_genres.
        ...
        if rows_to_update.height > 0:
            ...
            for row in rows_to_update.iter_rows(named=True):
                ...
                try:
                    ...
                    updated_row_count += 1
                    original_row = alib_df.filter(pl.col("rowid") == rowid).row(0, named=True)

                    if replacement_genre is not None:
                        cursor.execute(
                            "INSERT INTO changelog (rowid, column, old_value, new_value, timestamp, script) VALUES (?, ?, ?, ?, ?, ?)",
                            (rowid, "genre", original_row["genre"], replacement_genre, timestamp, script_name)
                        )

                    if replacement_style is not None:
                        cursor.execute(
                            "INSERT INTO changelog (rowid, column, old_value, new_value, timestamp, script) VALUES (?, ?, ?, ?, ?, ?)",
                            (rowid, "style", original_row["style"], replacement_style, timestamp, script_name)
                        )
