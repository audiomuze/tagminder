...
#     dbcursor.execute('''DROP TABLE IF EXISTS _INF_namesakes_albumartists;''')
#     dbcursor.execute('''DROP TABLE IF EXISTS _INF_namesakes_composers;''')
#     dbcursor.execute('''DROP INDEX IF EXISTS role_albumartists;''')
#     dbcursor.execute('''DROP TABLE IF EXISTS contributor_with_mbid;''')
#     dbcursor.execute('''DROP TABLE IF EXISTS _INF_namesakes_artists;''')
...
#     dbcursor.execute('''CREATE TABLE _INF_namesakes_albumartists AS SELECT *
#                                                           FROM (
...
#     dbcursor.execute('''CREATE TABLE _INF_namesakes_artists AS SELECT *
#                                                           FROM (
#                                                                WITH GET_NAMESAKES AS (
#                                                                        SELECT artist AS contributor,
#                                                                               musicbrainz_artistid
...
#     dbcursor.execute('''CREATE TABLE _INF_namesakes_composers AS SELECT *
#                                                           FROM (
#                                                                WITH GET_NAMESAKES AS (
#                                                                        SELECT composer AS contributor,
#                                                                               musicbrainz_composerid
...
#     # finally, write out any of the contributors in alib that don't have a match in _REF_mb_disambiguated to contributors_not_in_mbrainz
...
#     # get contributors in alib not in _REF_mb_disambiguated
...
def add_mb_entities():
    ''' if _REF_mb_disambiguated table exists adds musicbrainz identifiers to artists, albumartists, composers, engineers & producers
    # (we're adding musicbrainz_composerid, musicbrainz_engineerid and musicbrainz_producerid of our own volition for future app use) '''

    # check if the _REF_mb_disambiguated table exists.  If it's present then populate mbid's from it where records are currently without.  _REF_mb_disambiguated contains only names from mb_master that occor only once in that table
    ...
    if table_exists('_REF_mb_disambiguated'):
        ...
        dbcursor.execute('''CREATE INDEX IF NOT EXISTS ix__REF_lmb_disambiguated on _REF_mb_disambiguated(LOWER(entity));''')
            
        print(f"\nAdding musicbrainz identifiers to artists & albumartists by referencing MusicBrainz MBID table")
        opening_tally = tally_mods()


        # in all cases here we want to ensure that the existing mbid value in alib or the value written to alib corresponds with the relevant artist match.
        # we match lowercase as case differences would mean unnecessary mismatches
        # 
        #    update t1
        #       set id = t2.id
        #      from t2
        ...
        dbcursor.execute('''UPDATE alib
                               SET musicbrainz_albumartistid = _REF_mb_disambiguated.mbid
                              FROM _REF_mb_disambiguated
                             WHERE (_REF_mb_disambiguated.lentity = lower(alib.albumartist) AND 
                                    alib.musicbrainz_albumartistid IS NULL );''')

        # artist updates
        print('Updating artists')
        dbcursor.execute('''UPDATE alib
                               SET musicbrainz_artistid = _REF_mb_disambiguated.mbid
                              FROM _REF_mb_disambiguated
                             WHERE (_REF_mb_disambiguated.lentity = lower(alib.artist) AND 
                                   alib.musicbrainz_artistid IS NULL );''')

        # composer updates
        print('Updating composers')
        dbcursor.execute('''UPDATE alib
                               SET musicbrainz_composerid = _REF_mb_disambiguated.mbid
                              FROM _REF_mb_disambiguated
                             WHERE (_REF_mb_disambiguated.lentity = lower(alib.composer) AND 
                                   alib.musicbrainz_composerid IS NULL );''')

        # engineer updates
        print('Updating engineers')
        dbcursor.execute('''UPDATE alib
                               SET musicbrainz_engineerid = _REF_mb_disambiguated.mbid
                              FROM _REF_mb_disambiguated
                             WHERE (_REF_mb_disambiguated.entity = lower(alib.engineer) AND 
                                   alib.musicbrainz_engineerid IS NULL );''')

        # producer updates
        print('Updating producers')
        dbcursor.execute('''UPDATE alib
                               SET musicbrainz_producerid = _REF_mb_disambiguated.mbid
                              FROM _REF_mb_disambiguated
                             WHERE (_REF_mb_disambiguated.entity = lower(alib.producer) AND 
...
def add_multiartist_mb_entities():
    ...
    # check if the _REF_mb_disambiguated table exists.  If it's present then populate mbid's from it where records are currently without
    ...
    if table_exists('_REF_mb_disambiguated'):
        ...
        # dbcursor.execute('''CREATE INDEX IF NOT EXISTS ix_lartists ON alib(LOWER(artist)) WHERE artist IS NOT NULL;''')
        # dbcursor.execute('''CREATE INDEX IF NOT EXISTS ix_lcomposers ON alib(LOWER(composer)) WHERE composer IS NOT NULL;''')
        dbcursor.execute('''CREATE INDEX IF NOT EXISTS ix__REF_lmb_artists on _REF_mb_disambiguated(LOWER(entity));''')
        ...
        # only write changes if the current mbid value does not correspond with the mbid value in _REF_mb_disambiguated which is pulled directly from the MusicBrainz official dataset
        ...
        # select from multi_contributors only instances where the number of contributor names and number of mbid's are mismatched for re-processing
        ...
        if population > 0:
            ...
            for record in records:
                ...
                print(f"processing {baseline_contributor}")
                ...
                for contributor in contributors:
                    ...
                    dbcursor.execute('''SELECT mbid from _REF_mb_disambiguated where _REF_mb_disambiguated.lentity == (?);''', (contributor,))
            ...
            # # process artists
            print("Updating multi-contributor artist MBIDs")
            dbcursor.execute('''UPDATE alib
            ...
            print("Updating multi-contributor albumartist MBIDs")
            dbcursor.execute('''UPDATE alib
            ...
            # process composers
            print("Updating multi-contributor composer MBIDs")
            dbcursor.execute('''UPDATE alib
            ...
            # process engineers
            print("Updating multi-contributor engineer MBIDs")
            dbcursor.execute('''UPDATE alib
            ...
            # process producers
            print("Updating multi-contributor producer MBIDs")
            dbcursor.execute('''UPDATE alib
...
#             dbcursor.execute('''SELECT artist
#                                   FROM _REF_mb_disambiguated
#                                  WHERE (lower(_REF_mb_disambiguated.entity) = ?), (l_stored_entity))''')
...
def establish_alib_contributors():
    ...
    print('Generating _TMP_distinct_contributors table which holds distinct list of artist, albumartists, composers, writers, lyricists, engineers and producers present in your tags')
    ...
    dbcursor.execute("DROP TABLE IF EXISTS _TMP_distinct_contributors")
    ...
    # Ensure the _TMP_distinct_contributors table exists
    dbcursor.execute("CREATE TABLE IF NOT EXISTS _TMP_distinct_contributors (contributor TEXT PRIMARY KEY, lcontributor TEXT, processed INTEGER, mbid TEXT)")

    # Batch insert all unique, sorted entries from the merged list into a table for processing via string_grouper and other functions
    # this table is also used externally by the user to feed lists to string grouper and find possible namesakes, misspelled arist names, text case differences. irregular spacing, 'the ' etc.
    # the script to reference is grouper-taglib.py

    # Use executemany for efficient bulk insertion
    dbcursor.executemany('''INSERT OR IGNORE INTO _TMP_distinct_contributors (contributor, lcontributor, processed, mbid)
                            VALUES (?, ?, ?, NULL)''', data_to_insert)


    # index all_contributors
    dbcursor.execute('''CREATE INDEX IF NOT EXISTS ix_TMP_distinct_contributors ON _TMP_distinct_contributors(contributor)''')
    dbcursor.execute('''CREATE INDEX IF NOT EXISTS ix_TMP_ldistinct_contributors ON _TMP_distinct_contributors(lcontributor)''')

    dbcursor.execute('''select count(*) from _TMP_distinct_contributors;''')
...
#    string-grouper, populate the disambiguation table (_REF_vetted_contributors) with additional records based on the human curated 
...
#    All future ingeston is then run against _REF_vetted_contributors table to ensure that spurious data is autocorrected.
...
#########################################################################
    ...
    # this code block should be obsolete if we can take _TMP_distinct_contributors and always compare it against a list of vetted names from allmusic.com, then dealing only with new names via string grouper.
    # so we maintain a _REF_contributor_matched_on_allmusic table that contains vetted allmusic performers which is created by matching _TMP_distinct_contributors against _REF_contributor_matched_on_allmusic, then checking remmaining names 
    # against allmusic.com.  This way we iteratively keep the quality of _TMP_distinct_contributors robust, and also use _REF_contributor_matched_on_allmusic to do a lowercase match against _REF_mb_disambiguated to keep _REF_mb_disambiguated
    # aligned with how names are represented in allmusic.com
    # on the flipside, this is probably not the place to make this happen, because we'd be eliminating potential matches, giving rise to spurious entries.
    # what we should be doing is eliminating from the string_grouper output list is any entries in that list that also appear (case matched) in _REF_vetted_contributors so users don't waste time vetting data
    # they've already vetted.
    ...
    # # pull in only those contributors with commas that do not already appear in _REF_mb_disambiguated or _REF_contributor_matched_on_allmusic (because in both cases they're clearly already correct)
    # dbcursor.execute('''INSERT INTO
    ...
    #                               contributor
    #                             FROM
    #                               _REF_contributor_matched_on_allmusic
    #                           )
    #                         )
    ...
    # # pull in only those contributors with commas that do not already appear in _REF_mb_disambiguated (because they're clearly correct)
    # dbcursor.execute('''INSERT INTO
    ...
    df1 = pd.read_sql_query(f'SELECT rowid, contributor, FALSE FROM _TMP_distinct_contributors;', conn) # import all distinct contributors derived from alib
    ...
    # so what we now have is string_grouper's take on likely matches, based on specified similarity threshold
    ...
    # get all current_val for all records in _REF_vetted_contributors into a df so we can eliminate stuff we've already dealt with
    df2 = pd.read_sql_query(f'SELECT current_val from _REF_vetted_contributors ORDER by current_val;', conn)
    ...
    df3 = pd.read_sql_query(f'SELECT current_val, replacement_val from _REF_disambiguation_workspace WHERE status = 0 ORDER by current_val;', conn)

    # now drop rows from similarities if current_val', 'replacement_val' in a row matches a row in df3's current_val', 'replacement_val' values
    similarities[~similarities.set_index(['current_val', 'replacement_val']).index.isin(df3.set_index(['current_val', 'replacement_val']).index)]
...
def disambiguate_contributors():
    ...
    if not table_exists('_REF_vetted_contributors'):
    ...
    # dbcursor.execute('''SELECT current_val, replacement_val FROM _REF_vetted_contributors WHERE status = FALSE ;''')  # retrieve only disambiguation records not previously processed
    ...
    dbcursor.execute('''SELECT current_val, replacement_val FROM _REF_vetted_contributors;''')
    ...
    if disambiguation_count > 0:
        ...
        if df3.empty is False:
            ...
            df3.to_sql('_TMP_disambiguation_updates', conn, if_exists='replace', index=False)
            ...
            dbcursor.execute('''UPDATE 
                                ...
                                FROM 
                                  _TMP_disambiguation_updates AS source 
                                WHERE 
            ...
            dbcursor.execute('''UPDATE _REF_vetted_contributors set status = TRUE WHERE status = FALSE ;''')

            #strictly speaking _TMP_disambiguation_updates table is no longer required, so drop it.  However, ff you want to want to see what was replaced, retain it.
            # dbcursor.execute('''DROP TABLE _TMP_disambiguation_updates''')
            conn.commit()
...
def set_albumartist_caps():
    ...
    # this function compares albumartist names against _REF_mb_disambiguated and if the text case in alib differs it prefers the text case in _REF_mb_disambiguated
    # if no match exists, it compares it against a transformed albumartist calling on title_case(replace_demimiters(artist name))
    # it also creates and populates a table of albumartists not in _REF_mb_disambiguated for the user to peruse and check on to ensure they're correct
    # if they're not correct the user can correct them by adding their amendment to _REF_vetted_contributors, which is used elewhere to update _REF_mb_entities as well as all entries in alib
    ...
    if len(albumartists) > 0:
        for albumartist in albumartists:
            ...
            # Obv means mbrainz capitalisation must agree with what we see in allmusic via _REF_vetted_contributors table
            # so where there are differences we should update the musicbrainz table if that's important to you.
            # So some code should be written to id albumartists in current day alib where their text case differs from mbrainz and then mbrainz master _REF_mb_entities must be updated accordingly
            # Note, _REF_mb_entities and it's derived tables are already always checked against _REF_vetted_contributors when the script is run, so explicit changes that have been made via _REF_vetted_contributors
            # will always be reflected in _REF_mb_entities every time the script is run
            # if you are 100% sure your artist names appear exactly as you want them to in alib, you might choose to update _REF_mb_entities from distinct contributors in alib, albeit choosing not to code it at this juncture
...
def update_tags():
    ...
    # # from disambiguation table and allmusic vetted table, which in effect means those changes reflect how the artist name appears on allmusic.com
