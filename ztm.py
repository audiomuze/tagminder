    ...
    if len(badtags) > 0:
        ...
        for tagname in badtags:
            ...
            if not tagname[0].startswith('__'):
                ...
                dbcursor.execute("DROP INDEX IF EXISTS ix_spurious")
...
def nullify_empty_tags():
    ...
    for column in columns:
        ...
        if not column[0].startswith('__'):
            ...
            dbcursor.execute("DROP INDEX IF EXISTS ix_nullify;")
...
def trim_and_remove_crlf():
    ...
    for text_tag in text_tags:
        ...
        dbcursor.execute("DROP INDEX IF EXISTS crlf")
        dbcursor.execute("DROP INDEX IF EXISTS crlf1")
...
def set_apostrophe():
    ...
    for text_tag in text_tags:
        ...
        dbcursor.execute("DROP INDEX IF EXISTS ix_apostrophe;")
...
def title_keywords_to_subtitle():
    ...
    print('\n')
    dbcursor.execute('''CREATE INDEX IF NOT EXISTS ix_titles_subtitles ON alib(title, subtitle) WHERE title IS NOT NULL;''')
    opening_tally = tally_mods()
    ...
    dbcursor.execute("DROP INDEX IF EXISTS ix_titles_subtitles")
    print(f"|\n{tally_mods() - opening_tally} changes were processed")
...
def strip_live_from_titles():
    ...
    dbcursor.execute("DROP INDEX IF EXISTS titles")
    print(f"|\n{tally_mods() - opening_tally} changes were processed")
...
def merge_genre_style():
    ...
    opening_tally = tally_mods()
    dbcursor.execute("UPDATE alib SET genre = genre || '\\' || style WHERE style IS NOT NULL AND NOT INSTR(genre, style);")
    print(f"|\n{tally_mods() - opening_tally} changes were processed")


def split_album_version():
    ''' split album name and version fields, reverting album tag to album name '''
    print("\nRemoving VERSION tag from ALBUM tag")
    opening_tally = tally_mods()
    dbcursor.execute('''UPDATE alib
...
def establish_alib_contributors():
    ...
    print(f'Your library contains {records_generated} unique artists, albumartists composers, lyricists, writers, performers, engineers & producers')
...
#########################################################################
    ...
    df2 = pd.read_sql_query("SELECT current_val from _REF_vetted_contributors ORDER by current_val;", conn)
...
def disambiguate_contributors():
    ...
    if disambiguation_count > 0:
        ...
        df2 = df1.copy()
        print("Stats relating to records imported from alib table for processing:\n")
...
def unpad_tracks():
    ''' removes superflous padding from track i.e. undoes Picard's stupidity '''
...
def unpad_discnumbers():
    ''' removes superflous padding from discnumber i.e. undoes Picard's stupidity '''
...
def set_title_caps():
    ''' Applies RYM capitalisation rules to title tag '''
...
def set_album_caps():
    ''' Applies RYM capitalisation rules to album tag '''
...
def info_albums_with_duplicated_tracknumbers():
...
def info_albums_missing_tracknumbers():
...
def info_tracks_without_artist():
...
def export_changes():
    ...
    if changed_dirpaths:
        ...
        for dirpath in changed_dirpaths:
            ...
            dbcursor.execute("REPLACE INTO dirs_to_process (__dirpath) VALUES (?)", dirpath)
