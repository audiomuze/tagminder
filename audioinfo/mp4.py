...
class Tag(util.MockTag):
    ...
    def __deepcopy__(self, memo=None):
        ...
        cls.set_fundamentals(
            deepcopy(self.__tags),
            deepcopy(self.images),
            self.mut_obj,
            deepcopy(self.__freeform),
    ...
    def link(self, filename):
        ...
        if audio.tags:  # Not empty
            ...
            if "trkn" in keys:
                ...
                tags["totaltracks"] = convert(
                    "totaltracks", [z[1] for z in audio["trkn"]]
                )
                keys.remove("trkn")
            ...
            if "disk" in keys:
                ...
                tags["totaldiscs"] = convert(
                    "totaldiscs", [z[1] for z in audio["disk"]]
                )
                keys.remove("disk")
    ...
    def save(self):
        ...
        newtag = {}
        tuples = (("track", ["trkn", "totaltracks"]), ("disc", ["disk", "totaldiscs"]))
        tags = self.__tags
        for tag, values in tuples:
            if tag in tags:
                ...
                if values[1] in tags:
                    total = tags[values[1]]
                    newtag[values[0]] = [
                        (int(t), int(total)) for t, total in zip(denom, total)
                    ]
                else:
