"""
Purpose:
    Normalize and clean `genre`/`style` values in `alib`.

    Applies hard-coded replacements plus fuzzy matching (via `string_grouper`)
    against a validation/reference table (`_REF_genres`) in the master-data DB.

    Writes corrected values back to `alib`, increments `__sqlmodded`, and logs
...
CACHE_DIR = tm_config.get_cache_dir()
...
ALIB_SCHEMA = {
    ...
    "genre": pl.Utf8,
    "style": pl.Utf8,
    "__sqlmodded": pl.Int16,  # Unlikely to ever even get to 999
}
...
def build_corrected_mapping_optimized(
    ...
    if fuzzy_candidates:
        logging.info(
            f"Running optimized string_grouper on {len(fuzzy_candidates)} candidates..."
        )
        fuzzy_matches = optimized_string_grouper_matching(
            fuzzy_candidates, valid_tags, similarity_threshold
        )
    ...
    if invalid_tags:
        logging.info(
            f"Found {len(invalid_tags)} invalid tags that will be filtered out"
        )
...
def intelligent_pre_filter(
    raw_tags: List[str], valid_tags: Set[str]
    ...
    for k, v in HARD_CODED_REPLACEMENTS.items():
        ...
        if v in valid_tags or any(
            part.strip() in valid_tags for part in v.split(DELIMITER)
        ):
    ...
    for tag in raw_tags:
        ...
        # 4. Try some common variations before fuzzy matching
            ...
            exact_matches[tag] = valid_tags_lower[
                tag_lower.replace("music", "").strip()
            ]
    ...
    logging.info(
        f"Intelligent filtering: {len(exact_matches)} exact matches, {len(fuzzy_candidates)} fuzzy candidates"
    )
...
def process_tags_vectorized(
    df: pl.DataFrame, tag_mapping: Dict[str, str]
) -> pl.DataFrame:
    """
    ...
    result = (
        ...
        .with_columns(
            [
                # Map genre tags
                pl.col("genre_tags")
                .map_elements(map_tag_list, return_dtype=pl.List(pl.String))
                .alias("mapped_genre"),
                # Map style tags
                pl.col("style_tags")
                .map_elements(map_tag_list, return_dtype=pl.List(pl.String))
...
def optimized_string_grouper_matching(
    ...
    for i in range(0, len(fuzzy_candidates), batch_size):
        ...
        logging.info(
            f"Processing string_grouper batch {i // batch_size + 1}/{(len(fuzzy_candidates) - 1) // batch_size + 1}"
        )
    ...
    changes_df = (
        ...
        .with_columns(
            [
                ...
                (pl.col("new_genre_norm") != pl.col("genre_norm")).alias(
                    "genre_changed"
                ),
                (pl.col("new_style_norm") != pl.col("style_norm")).alias(
                    "style_changed"
                ),
            ]
        )
    ...
    for row in changes_df.iter_rows(named=True):
        if row["genre_changed"]:
            changelog_entries.append(
                {
                    "alib_path": row.get("__path") or str(row["rowid"]),
                    "column": "genre",
...
def merge_genre_style_vectorized(df: pl.DataFrame) -> pl.DataFrame:
    ...
    df = df.with_columns(
        [pl.col("new_genre").cast(pl.Utf8), pl.col("new_style").cast(pl.Utf8)]
    )
    ...
    df = df.with_columns(
        [
            ...
            pl.when(
                pl.col("new_genre").is_null()
                | (pl.col("new_genre").str.len_chars() == 0)
            )
            ...
            pl.when(
                pl.col("new_style").is_null()
                | (pl.col("new_style").str.len_chars() == 0)
            )
            .then(pl.lit([], dtype=pl.List(pl.Utf8)))
    ...
    return df.drop(
        ["genre_list", "style_list", "genre_clean", "style_clean", "merged_tags"]
    )
...
def batch_database_updates(
    conn: sqlite3.Connection, updates: List[Dict], changelog_entries: list[tm_db.ChangelogEntry]
) -> bool:
    ...
    try:
        ...
        if updates:
            update_sql = tm_db.build_update_sql(
                table=ALIB_TABLE,
                set_cols=["genre", "style"],
            )
            cursor.executemany(
                update_sql,
                [
                    (u["genre"], u["style"], int(u["__sqlmodded"] or 0), u["rowid"])
                    for u in updates
                ],
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Clean and standardize genre/style tags"
    )
    parser.add_argument(
        "--db",
        metavar="PATH",
    ...
    parser.add_argument(
        ...
        help="Path to master-data SQLite database for _REF_genres (default: tagminder.toml [master_data].path)",
    )
    parser.add_argument(
        "--merge-genres-styles",
        action="store_true",
    ...
    if not args.master_db:
        parser.error(
            "No master-data DB path resolved: set tagminder.toml [master_data].path or pass --master-db PATH"
        )
    ...
    try:
        ...
        if not Path(args.master_db).exists():
            logging.error(f"Master database file does not exist: {args.master_db}")
            return

        conn = tm_db.connect(args.db)
        master_conn = tm_db.connect(args.master_db)
        ...
        valid_tags = import_valid_tags(master_conn)
        ...
        query = f"""
            SELECT rowid, __path, genre, style, COALESCE(__sqlmodded, 0) AS __sqlmodded
            FROM {ALIB_TABLE}
        ...
        def create_changelog_entries_vectorized(
            df: pl.DataFrame,
            ...
            changes_df = (
                ...
                .with_columns(
                    [
                        # Overall change flag
                        (pl.col("genre_changed") | pl.col("style_changed")).alias(
                            "has_changes"
                        )
                    ]
                )
            ...
            # Generate changelog entries for changed rows.
            # Keep the existing change-detection semantics: treat None and "" as equivalent.
        ...
        while True:
            ...
            df = pl.DataFrame(
                {
                    ...
                    "__sqlmodded": tm_polars.series_sqlmodded([r[4] for r in rows]),
                }
            )
            ...
            if len(update_batch) >= BATCH_SIZE:
                if batch_database_updates(conn, update_batch, changelog_batch):
                    ...
                    logging.info(
                        f"Processed {total_processed:,} rows, updated {total_updated:,}"
                    )
        ...
        rows_per_second = (
            total_processed / duration.total_seconds()
