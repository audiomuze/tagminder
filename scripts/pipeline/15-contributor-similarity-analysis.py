"""
Purpose:
    Build a consolidated list of contributors from multiple `alib` columns and
    use `string_grouper` to identify potential duplicates that may represent
    the same entity.

    Stores a workspace table for inspection and supports optional use of vetted
    contributor values.
...
#         return pl.DataFrame({"contributor": [], "lpreferred__artist_name": []})
...
#         pl.col("contributor").str.to_lowercase().alias("lpreferred__artist_name")
#     ])
...
def extract_and_process_contributors(conn: sqlite3.Connection, write_distinct_csv: bool = False) -> pl.DataFrame:
    ...
    contributors_df = tm_polars_db.sqlite_to_polars(
        conn,
        f"""
        SELECT {contributor_select}
        FROM alib
    ...
    # Handle delimited entries using vectorized string operations
    # Split on Tagminder's multi-value delimiter and explode to separate rows
    processed_df = melted_df.with_columns([
        pl.col("contributor").str.split(DELIMITER).alias("contributor_parts")
    ]).explode("contributor_parts").with_columns([
        pl.col("contributor_parts").str.strip_chars().alias("contributor")
    ]).filter(
        pl.col("contributor").is_not_null() &
...
def get_processed_contributors_vectorized(conn: sqlite3.Connection) -> Set[str]:
    ...
    if table_exists(conn, '_REF_vetted_contributors'):
        vetted_df = tm_polars_db.sqlite_to_polars(
            conn,
    ...
    if table_exists(conn, '_REF_contributors_workspace'):
        workspace_df = tm_polars_db.sqlite_to_polars(
            conn,
            """
...
def create_workspace_entries_vectorized(similarities_df: pl.DataFrame) -> pl.DataFrame:
    ...
    group_stats = grouped_df.group_by("group_id").agg(
        ...
        pl.concat_list(["left_contributor", "right_contributor"])
        .list.explode(keep_nulls=False, empty_as_null=False)
        .unique()
        .alias("group_contributors"),
...
def main():
    ...
    try:
        ...
        master_conn = conn if master_db_path == args.db else tm_db.connect(master_db_path)
        ...
        processed_contributors = get_processed_contributors_vectorized(master_conn)
        ...
        if similarities_df.height > 0:
            ...
            update_workspace_optimized(master_conn, workspace_df, save_csv=args.csv)
        ...
        if master_conn is not None and master_conn is not conn:
            master_conn.close()
        if conn is not None:
            conn.close()
