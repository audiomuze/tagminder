...
table = 'sg_contributors'

# Import all entries in table to a dataframe

df = pd.read_sql_query(f'SELECT rowid, artist, FALSE FROM {table};', db) # import artists, performers, albumartists and composers

matches = match_strings(df['artist'], min_similarity=0.85)
# Look at only the non-exact matches:
similarities = matches[matches['left_artist'] != matches['right_artist']].head(None)
