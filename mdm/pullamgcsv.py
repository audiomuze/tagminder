...
def cleanup_previous_run(db_name: str = "dbtemplate.db"):
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='allmusic_reference_data'"
        )
            ...
            while True:
                try:
                    ...
                    for col_name, dtype in zip(chunk.columns, chunk.dtypes):
                        if dtype != pl.String:
                            raise ValueError(
                                f"Column {col_name} has type {dtype}, expected String. Type inference occurred!"
                            )
