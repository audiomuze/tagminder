...
def insert_valid_rows(conn: sqlite3.Connection, df: pl.DataFrame):
    ...
    # Vectorized Unicode hyphen replacement for contributor column
    df = df.with_columns(
        pl.col("contributor").str.replace_all("\u2010", "-").alias("contributor")
    )
...
if __name__ == "__main__":
    ...
    print(
        "Hyphen normalization: Replacing Unicode U+2010 (HYPHEN) with Unicode U+002D (HYPHEN-MINUS) in contributor column"
    )
