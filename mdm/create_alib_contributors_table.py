...
def extract_all_contributors(conn: sqlite3.Connection) -> pl.DataFrame:
    ...
    df = pl.DataFrame({"contributor": [row[0] for row in rows]})
...
def split_and_deduplicate_contributors(df: pl.DataFrame) -> pl.DataFrame:
    ...
    contributors = (
        ...
        .select([pl.col("split_contributors").str.strip_chars().alias("contributor")])
        .filter((pl.col("contributor").is_not_null()) & (pl.col("contributor") != ""))
        .unique()
        .sort("contributor")
    )
...
def add_reference_check(
    contributors_df: pl.DataFrame, conn: sqlite3.Connection
) -> pl.DataFrame:
    """
    ...
    try:
        ...
        result = (
            contributors_df.with_columns(
                [pl.col("contributor").str.to_lowercase().alias("lcontributor")]
            )
            .join(reference_df, on="lcontributor", how="left")
            .select(
                [
                    "contributor",
...
def write_to_database(df: pl.DataFrame, conn: sqlite3.Connection) -> None:
    ...
    try:
        ...
        records = df.select(["contributor", "in_allmusic_reference_data"]).rows()

        # Batch insert all records
        cursor.executemany("INSERT INTO alib_contributor_names VALUES (?, ?)", records)
        ...
        conn.commit()
        logging.info(
            f"Successfully wrote {len(records)} contributors to alib_contributor_names"
        )
