...
ATTRIBUTES = ["length", "accessed", "size", "created", "modified", "filetype"]

COVER_KEYS = {"cover art (front)": 3, "cover art (back)": 4}


class DefaultHeaderError(RuntimeError):
    pass
    ...
    class APEv2Base(MockTag):
        ...
        def __setitem__(self, key, value):
            ...
            # elif isempty(value):
                ...
                if value is None:
                    return  # CHANGED: Only skip if value becomes None after conversion
        ...
        def info(self):
            ...
            attr = [("Channels", "channels"), ("Version", "version")]
            for k, v in attr:
                try:
        ...
        def link(self, filename):
            ...
            no_header = DefaultHeaderError if header_error is None else header_error
        ...
        def save(self):
            ...
            for field, value in usertags(self.__tags).items():
                try:
                    ...
                    pass
            toremove = [
                z for z in audio if z not in newtag and audio[z].kind in (TEXT, BINARY)
            ]
            for z in toremove:
                del audio[z]
...
wv_base = get_class(
    WavPack, "WavPack", ATTRIBUTES + ["frequency", "bitrate"], WavPackHeaderError
)
...
filetypes = [
    (APEv2File, Tag, "APEv2"),
    (MonkeysAudio, MonkeysAudioTag, "APEv2", ["ape", "apl"]),
